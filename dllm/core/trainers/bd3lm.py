...
def _create_bd3lm_attention_mask(
    b, h, q_idx, kv_idx, block_size=None, n=None, x0_first=False
):
    """
    ...
    Args:
        ...
        n: Half-sequence length (each segment has n tokens).
        x0_first: If True, concatenation order is [x0, xt] instead of [xt, x0].

    Returns:
        A boolean attention mask.
    """

    # Segment membership (position-based, order-independent)
    is_second_q = q_idx >= n
    is_second_kv = kv_idx >= n

    # x0 flag depends on concatenation order
    if x0_first:
        x0_flag_q = ~is_second_q  # x0 is first segment [0, n)
        x0_flag_kv = ~is_second_kv
    else:
        x0_flag_q = is_second_q  # x0 is second segment [n, 2n) (default)
        x0_flag_kv = is_second_kv

    # Block indices: always based on segment position (first vs second)
    block_q = torch.where(is_second_q, (q_idx - n) // block_size, q_idx // block_size)
    block_kv = torch.where(
        is_second_kv, (kv_idx - n) // block_size, kv_idx // block_size
    )
