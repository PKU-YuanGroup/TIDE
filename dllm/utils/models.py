...
def get_default_device_map() -> dict | None:
    """
    Return a safe device_map for model loading.

    - Avoid device_map when using DeepSpeed/FSDP (can conflict with sharding).
    - Avoid device_map when ZeRO-3 is enabled.
    - Fall back to cuda:0 if the accelerate state isn't fully initialized.
    """
    ...
    try:
        ...
        blocked = {DistributedType.DEEPSPEED, DistributedType.FSDP}
        if hasattr(DistributedType, "FSDP2"):
            blocked.add(DistributedType.FSDP2)
        if state.distributed_type in blocked:
            return None
...
def get_model(
    ...
    **kwargs,
    ...
    dtype_str = kwargs.get("dtype", getattr(model_args, "dtype", "bfloat16"))
    ...
    if attn_implementation == "flex_attention":
        try:
            ...
            state = accelerate.PartialState()
            force_flex = os.getenv("FORCE_FLEX_ATTENTION", "").lower() in (
                "1",
                "true",
                "yes",
            )
            if (
                not force_flex
                and state.distributed_type == DistributedType.DEEPSPEED
            ):
                print_main(
                    "flex_attention + DeepSpeed is unstable; falling back to sdpa. "
                    "Set FORCE_FLEX_ATTENTION=1 to override."
                )
